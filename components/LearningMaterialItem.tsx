...
import { VideoCameraIcon, LinkIcon, ViewGridAddIcon, DocumentIcon, BookOpenIcon, CodeIcon, AcademicCapIcon } from '@heroicons/react/outline';
...
const LearningMaterialItem = ({ resource }: LearningMaterialItemProps) => {
  const { title, description, type, difficulty, thumbnail, url, tags = [] } = resource;
  ...
  const urlToRender = new URL(url);

  return (
    <a href={`${url}?ref=virtualeda`} target="_blank" className={`block bg-gradient-to-br from-gray-500 to-resource-${type}-dark p-1 rounded hover:from-gray-100`} rel="noreferrer">
      <div className="bg-gray-800 text-left text-gray-400 flex flex-col h-full">
        <div className="bg-black  mb-4">
          ...
          {videoID && thumbnailURL && <img src={thumbnailURL} className="w-full h-64 object-contain " />}
          {!thumbnail && !videoID && (
            <div className="mx-auto w-full py-14">
              <TypeIcon className="w-full h-32 mx-auto opacity-10 " />
            </div>
          )}
          {/* <div className="text-xs mt-2 text-gray-600 block">{urlToRender.origin}</div> */}
        </div>
